...
public class TwoSumTests
{
    ...
    [Theory]
    [MemberData(nameof(GetCorrectDataSets))]
    ...
    [Theory]
    [MemberData(nameof(GetCorrectDataSets))]
    public void V2Correct(int[] numbers, int target, int[] expected)
    {
        int[] result = TwoSumV2.Execute(numbers, target);
        Assert.NotNull(result);
        Assert.Equal(expected, result);
    }

    public static IEnumerable<object[]> GetCorrectDataSets()
    {
        return new[]
        {
